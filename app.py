...
if files:
    ...
    with st.sidebar:
        if len(final_list) > 0:
            if st.button("🚀 导出全部 (ZIP)", use_container_width=True):
                zip_buf = io.BytesIO()
                with zipfile.ZipFile(zip_buf, 'w') as zf:
                    for i, itm in enumerate(final_list):
                        data, ext = process_engine(itm, conf)
                        name = getattr(itm, 'filename', getattr(itm, 'name', f"output_{i}.jpg"))
                        zf.writestr(f"{name.split('.')[0]}.{ext.lower()}", data)
                st.download_button("📥 点击下载 ZIP", zip_buf.getvalue(), f"Batch_{datetime.now().strftime('%H%M')}.zip", use_container_width=True)
else:
    st.info("💡 请上传图片或 PDF 开始处理。")
