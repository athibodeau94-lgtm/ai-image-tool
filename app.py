...
def engine(item, cfg, is_preview=False):
    ...
    if ext == "JPEG":
        ...
        while q > 10:
            ...
            if is_preview or cfg['limit'] == 0 or out_io.tell() <= cfg['limit'] * 1024: break
            q -= 5
    else:
...
with st.sidebar:
    ...
    c1, c2 = st.columns(2)
    with c1:
        res = st.selectbox("分辨率", ["1920*1080", "1000*600", "800*800"])
        tw, th = map(int, res.split('*'))
    with c2:
        vol = st.selectbox("体积控制", ["不限制", "500KB", "1MB"])
        limit = {"不限制":0, "500KB":500, "1MB":1024}.get(vol)
    ...
    br, sh = st.slider("亮度", 0.5, 1.5, 1.05), st.slider("锐化", 1.0, 4.0, 1.8)
...
if files:
    ...
    cfg = {'size':(tw,th), 'limit':limit, 'bg':bg_m, 'color':c_val, 'br':br, 'sh':sh}
    ...
    if st.sidebar.button("🚀 导出 ZIP", use_container_width=True):
        ...
        st.sidebar.download_button("📦 点击下载", z_buf.getvalue(), "output.zip", use_container_width=True)
else:
    st.info("💡 请上传文件开始。")
