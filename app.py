...
# 初始化：增加微调参数存储 adj{文件名: {zoom, off_x, off_y}}
...
def process_engine(img_input, config, adj_params=None, is_preview=False):
    try:
        ...
        off_x = (adj_params.get('off_x', 0) // 2) if (adj_params and is_preview) else adj_params.get('off_x', 0) if adj_params else 0
        off_y = (adj_params.get('off_y', 0) // 2) if (adj_params and is_preview) else adj_params.get('off_y', 0) if adj_params else 0
        ...
        # 主体保持比例居中 + 应用微调挪动
        main_w, main_h = int(render_w * zoom), int(render_h * zoom)
        img_main = img.copy()
        img_main.thumbnail((main_w, main_h), Image.Resampling.LANCZOS)
        
        offset = ((render_w - img_main.width) // 2 + off_x, (render_h - img_main.height) // 2 + off_y)
        bg.paste(img_main, (int(offset[0]), int(offset[1])), img_main)
        res = bg
...
with left_col:
    ...
    with st.expander("🛠️ 规格设置", expanded=True):
        ...
        # 自动切换体积控制
        vol_list = ["不限制", "500KB", "1MB", "自定义"]
        def_vol_idx = None # 默认为空（待选择）
        if res_label in ["聚合标准 (1920*1080)", "Kiosk/Emenu标准 (5:3)"]:
            def_vol_idx = 1 # 自动跳转 500KB
        
        vol_opt = st.selectbox("体积控制", vol_list, index=def_vol_idx, placeholder="待选择...")
        kb = 0
        if vol_opt == "500KB": kb = 500
        elif vol_opt == "1MB": kb = 1024
        elif vol_opt == "自定义": kb = st.number_input("KB", 1, 10000, 500)

    with st.expander("🎨 视觉微调", expanded=True):
        auto_crop = st.toggle("多主体识别拆分", value=False) # --- 功能3: PDF/多主体逻辑保留 ---
        ...
        if st.session_state.editing_key:
            st.markdown("---")
            st.write(f"📍 **正在微调位置**: {st.session_state.editing_key}")
            ek = st.session_state.editing_key
            if ek not in st.session_state.adj: st.session_state.adj[ek] = {'zoom':1.0, 'off_x':0, 'off_y':0}
            st.session_state.adj[ek]['zoom'] = st.slider("放大/缩小", 0.5, 2.0, st.session_state.adj[ek]['zoom'])
            st.session_state.adj[ek]['off_x'] = st.slider("左右挪动", -tw//2, tw//2, st.session_state.adj[ek]['off_x'])
            st.session_state.adj[ek]['off_y'] = st.slider("上下挪动", -th//2, th//2, st.session_state.adj[ek]['off_y'])
            if st.button("✅ 保存位置", use_container_width=True):
                st.session_state.editing_key = None
                st.rerun()
    ...
    if not res_label:
            ...
            if f.name.lower().endswith('.pdf') and PDF_SUPPORT:
                ...
                for i, p in enumerate(pages):
                    if auto_crop:
                        for idx, dish in enumerate(smart_extract_multiple_subjects(p)):
                            dish.unique_id = f"{f.name}_P{i+1}_{idx}"; final_list.append(dish)
                ...
                if auto_crop:
                    for idx, dish in enumerate(smart_extract_multiple_subjects(img_obj)):
                        dish.unique_id = f"{f.name}_{idx}"; final_list.append(dish)
                else: 
        ...
        with st.container(height=550):
            ...
            for i, item in enumerate(final_list):
                ...
                with cols[i % 3]:
                    is_edit = (st.session_state.editing_key == u_id)
                    if is_edit: st.markdown('<div class="edit-active">', unsafe_allow_html=True)
                    ...
                    if p_bytes:
                        ...
                        if st.button("🎯 挪动/缩放", key=f"btn_{u_id}"):
                            st.session_state.editing_key = u_id
                            st.rerun()
                    if is_edit: st.markdown('</div>', unsafe_allow_html=True)

        if st.button("🚀 一键打包下载 (包含微调)", type="primary", use_container_width=True):
            zip_buf = io.BytesIO()
            with zipfile.ZipFile(zip_buf, 'w') as zf:
                for item in final_list:
                    u_id = getattr(item, 'unique_id', "img")
                    f_adj = st.session_state.adj.get(u_id, {'zoom':1.0, 'off_x':0, 'off_y':0})
                    data, _ = process_engine(item, conf, adj_params=f_adj, is_preview=False)
                    zf.writestr(f"{u_id}.jpg", data)
            st.download_button("📥 获取 ZIP", zip_buf.getvalue(), file_name=f"Batch_{datetime.now().strftime('%H%M')}.zip", use_container_width=True)
