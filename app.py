...
with st.sidebar:
    ...
    # --- 【优化：体积控制双单位支持】 ---
    vol_opt = st.selectbox("体积控制", ["不限制", "500KB", "1MB", "2MB", "自定义"])
    kb_val = 0
    if vol_opt == "自定义":
        col_v, col_u = st.columns([2, 1])
        with col_u:
            unit = st.radio("单位", ["KB", "MB"], horizontal=True)
        with col_v:
            if unit == "KB":
                kb_val = st.number_input("数值", 10, 102400, 500)
            else:
                mb_val = st.number_input("数值", 0.1, 100.0, 1.0, step=0.1)
                kb_val = int(mb_val * 1024)
    elif vol_opt != "不限制":
        mapping = {"500KB": 500, "1MB": 1024, "2MB": 2048}
        kb_val = mapping.get(vol_opt, 0)
...
if files:
    ...
    with col2:
        ...
        st.write(f"📏 目标体积: {'不限' if kb_val==0 else f'{kb_val} KB'}")
